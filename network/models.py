...
class Post(models.Model):
    ...
    def __str__(self):
        return f'"Post {self.id}, {self.content[:15] + "..."}" posted by {self.user}'
...
class Comment(models.Model):
    post            = models.ForeignKey(Post, on_delete=models.CASCADE, related_name="post_commented_on")
    user            = models.ForeignKey(User, null=True, on_delete=models.SET_NULL)
    comment_text    = models.TextField(null=True, blank=True, max_length=300)
    timestamp       = models.DateTimeField(auto_now_add=True)
